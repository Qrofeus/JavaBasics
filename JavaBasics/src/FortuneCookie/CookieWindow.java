...
public class CookieWindow extends JFrame{
    public CookieWindow() {
        ...
        cracker=new JButton("Crack a Cookie");
        this.add(cracker,BorderLayout.NORTH);

        this.setVisible(true);
        setLayout(new BorderLayout());

        JPanel panel =new JPanel();
        JLabel cookieMessage=new JLabel();

        Random rnd=new Random();

        cracker.addActionListener(e -> cookieMessage.setText(cookies.get(rnd.nextInt(cookies.size()))));
        panel.add(cookieMessage);
